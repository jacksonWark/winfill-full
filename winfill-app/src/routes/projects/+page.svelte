...
{#each data as section}
    <ProjectCategory name={section.name} id={section.id} text={section.text} numImages={section.images.length} imagesPerRow={section.imagesPerRow}/>
    {#each section.images as image, i}
        <ProjectTile alt={image.alt} source={imagePath+image.source} route={`/projects/${section.id}/${image.route}`} imagesPerRow={section.imagesPerRow} index={i} />
    {/each}
{/each}